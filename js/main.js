...
function popupOpen(){
    ...
    // 팝업 층별 안내
    let $popupImg = document.querySelector('#mainfo .inner .parkingPopup .floorSelect img')
    if($popupImg){
...
let menu_list_arr=[
    {menuid : '홈 화면' , keyword: '메인 , 홈 , 처음 , 첫 , 병원' , linktext :'index.html'},
    {menuid : '의료진 메인' , keyword: '메인 , 홈 , 처음 , 첫 , 의사 , 의료진' , linktext :'index-docter.html'},
    {menuid : '의료진 찾기' , keyword: '의료진 , 의사 , 찾기 , 검색 , 사람' , linktext :'SearchDoc.html'},
    {menuid : '로그인' , keyword: '로그인 , 회원가입 , 비회원 , 예약' , linktext :'login1.html'},
    {menuid : '주차안내도' , keyword: '주차 , 차량 , 길 , 시설 , 편의 문병 , 방문', linktext :'main2parkingInfo.html' },
    {menuid : '예약안내' , keyword: '요양 , 예약 , 진료 , 입원 , 방문 , 전화 , 인터넷 , 앱 , 의뢰 , 취소' , linktext :'main1ResevationInfo.html'},
    {menuid : '진료의뢰서 발급' , keyword: '응급 , 법률 , 장애인 , 제출 ', linktext :'main2treatRequest.html' },
    {menuid : '제증명 FAQ' , keyword: '질문 , 궁금 , 질병명 , 상병코드 , 진단 , 입원 , 진료 , 방문 , 전화 , 취소 , 제출 , 수술 ' , linktext :'work1.html'},
    {menuid : '병원개요' , keyword: '소개 , 의학 , 설립 , 연구  , 진료 , 질문 , 궁금  , 방문 , 시설 ' , linktext :'work3.html'},
    {menuid : '고객헌장' , keyword: '인쇄 , 소개 , 다운로드  , 병원 , 설립 , 연구 ' , linktext :'work5.html'},
]
...
function searchmenu_but(SearchText) {
    ...
    menu_list_arr.forEach((it, j) => {
        if (
            it.menuid.includes(SearchText) ||
            it.keyword.includes(SearchText)
        ) {
        ...
        the_registration_process_li_button.forEach(item => {
         ...
         let timeid = setInterval(() => {
            ...
            if (num >= 4) {
             ...
             if (the_registration_process_li_button.length === 0 || the_registration_process_li_i.length === 0) {
                return;
              }
...
let Resident_registration_number=document.getElementById('Resident_registration_number')
let Patient_number=document.getElementById('Patient number')
let registration_number_tr=document.querySelector('.Identification_check .registration_number_tr')
let consultation_number_tr=document.querySelector('.Identification_check .consultation_number_tr')
